...
def leer_tablero_fich(nombre_fich):
    ...
    try:
        ...
        with open(nombre_fich) as f:
            ...
            # NOTA: tal como está hecho, el bucle se podría haber
            # sustituido por simplemente f.read(), que ya lee todo
            # "junto" y hubiera funcionado (o casi, salvo si los
            # saltos de línea no son de la forma esperada)
            for line in f:
                line = line.strip()  # quitar espacios ini/fin y \n final
                line = line.replace(" ", "")  # Opc.: quitar espacios intermedios
                if line != "":  # ignorar las vacías
                    # Voy construyendo una gran cadena de nuevo con todas las
                    # líneas "juntas", porque así puedo aprovechar la función
                    # que ya existía: leer_tablero()
                    cad = cad + line + "\n"
    finally:
        # El trabajo de convertir de 1 única cadena a matriz ya estaba hecho...
        m = leer_tablero(cad)
    return m
